    ...
    <>
      ...
      <nav className="navbar">
        ...
        <ul className="nav-links desktop-nav">
          ...
          <li><Link to="/news" className={isActive('/news')}>{t('news')}</Link></li>
          ...
          <li>
            <button className="lang-toggle" onClick={toggleLanguage}>
              {language === 'en' ? '🇪🇹 አማ' : language === 'am' ? '🇪🇹 ኦሮ' : '🇬🇧 EN'}
            </button>
          </li>
        </ul>
      </nav>
      ...
      <div className={`mobile-drawer ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="drawer-links">
          ...
          <Link to="/news" className={isActive('/news')} onClick={closeMenu}>
            <span className="drawer-icon">📰</span> {t('news')}
          </Link>
          ...
          <Link to="/privacy-policy" className={isActive('/privacy-policy')} onClick={closeMenu}>
            <span className="drawer-icon">📜</span> {language === 'en' ? 'Privacy Policy' : language === 'am' ? 'የግላዊነት ፖሊሲ' : 'Imaammata Dhuunfaa'}
          </Link>
          <div className="drawer-divider"></div>
          <button className="drawer-lang-btn" onClick={toggleLanguage}>
            <span className="drawer-icon">{language === 'en' ? '🇪🇹' : language === 'am' ? '🇪🇹' : '🇬🇧'}</span>
            {language === 'en' ? 'አማርኛ' : language === 'am' ? 'Afaan Oromoo' : 'English'}
          </button>
        </div>
      ...
      <footer className="footer">
        <div className="footer-content">
          <div className="footer-brand">
            ...
            <div className="footer-contact">
              ...
              <p>📞 +251 911 031 884 | +251 905 841 982 | +251 915 840 037</p>
            </div>
          ...
          <div className="footer-links">
            ...
            <ul>
              ...
              <li><Link to="/privacy-policy">{language === 'en' ? 'Privacy Policy' : language === 'am' ? 'የግላዊነት ፖሊሲ' : 'Imaammata Dhuunfaa'}</Link></li>
            </ul>
          </div>
          <div className="footer-links">
            <h4>{t('services')}</h4>
        ...
        <div className="footer-bottom">
          <p>&copy; {new Date().getFullYear()} <span>All Things</span> Marketing Agency. {t('allRightsReserved')}</p>
        </div>
      </footer>
    </>
  )
